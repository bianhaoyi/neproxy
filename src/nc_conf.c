...
static struct command conf_commands[] = {
    ...
    { string("server_connect_timeout"),
      conf_set_num,
      offsetof(struct conf_pool, server_connect_timeout) },
    ...
    { string("cachedb"),
      conf_set_bool,
      offsetof(struct conf_pool, cachedb) },
...
conf_pool_each_transform(void *elem, void *data)
{
    ...
    sp->cachedb = cp->cachedb ? 1 : 0;
    sp->tcpkeepalive = cp->tcpkeepalive ? 1 : 0;
    sp->timeout = cp->timeout;
    sp->server_connect_timeout = cp->server_connect_timeout;
...
conf_dump(struct conf *cf)
{
    ...
    for (i = 0; i < npool; i++) {
        ...
        log_debug(LOG_VVERB, "  server_connect_timeout: %d", cp->server_connect_timeout);
        ...
        log_debug(LOG_VVERB, "  cachedb: %d", cp->cachedb);
...
conf_validate_pool(struct conf *cf, struct conf_pool *cp)
{
    ...
    if (cp->server_connect_timeout == CONF_UNSET_NUM) {
        cp->server_connect_timeout = CONF_DEFAULT_SERVER_CONNECT_TIMEOUT;
    }
    ...
    if (cp->cachedb == CONF_UNSET_NUM) {
        cp->cachedb = CONF_DEFAULT_CACHEDB;
    }
