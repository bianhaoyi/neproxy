...
server_timeout(struct conn *conn)
{
    ...
    server = conn->owner;
    pool = server->owner;
    if (pool->cachedb && conn->connecting) {
        conn->connect_timeout = true;
        return pool->server_connect_timeout;
    }
...
server_conn(struct server *server)
{
    ...
    if (pool->cachedb) {
        log_debug(LOG_INFO, "cachedb server ns_conn_q %d", server->ns_conn_q);
    ...
    if (pool->cachedb && conn->nofree) {
        /* int i = 0; */
        /* for (i=0; i<server->ns_conn_q; i++) { */
        /*     TAILQ_REMOVE(&server->s_conn_q, conn, conn_tqe); */
        /*     TAILQ_INSERT_TAIL(&server->s_conn_q, conn, conn_tqe); */
        /*     conn = TAILQ_FIRST(&server->s_conn_q); */
        /*     ASSERT(!conn->client && !conn->proxy); */
        ...
        /* if (i >= server->ns_conn_q) { */
        /*     log_debug(LOG_INFO, "cachedb conn fail"); */
        /*     return NULL; */
        /* } */
        if (server->ns_conn_q < pool->server_connections) {
            log_debug(LOG_INFO, "server ns_conn_q %d", server->ns_conn_q);
            conn = conn_get(server, false, pool->redis); 
            if (conn != NULL) {
                conn->nofree = 1;
            }
                 ...
                 uint32_t keylen)
{
    ...
    if (status != NC_OK) {
        log_debug(LOG_INFO, "server: failed to update pool");
        return NULL;
    }
    ...
    if (server == NULL) {
        log_debug(LOG_INFO, "server: failed to pick server");
        return NULL;
    }
    ...
    conn = server_conn(server);
    if (conn == NULL || conn == -1) {
        log_debug(LOG_INFO, "server: failed to pick conn");
        return conn;
    }

    status = server_connect(ctx, server, conn);
    if (status != NC_OK) {
        log_debug(LOG_INFO, "server: failed to connect");
